...
declare var $: any;
@Component({
  selector: 'app-ifsc-city',
  templateUrl: './bank-city.component.html',
  styleUrls: ['./bank-city.component.css']
})
export class IfscCityComponent implements OnInit {
...
changeText: boolean;
  ...
  GetAllCityName(){
    this.routeSub = this.activeroute.params.subscribe(params => {
      ...
      this.titleService.setTitle('Find IFSC & MICR Code For ' + this.Meta_bankname +' in ' + this.Meta_statename + ' | Homes247');
      this.meta.updateTag({
        name: 'description',
        content: 'Find Details of ' + this.Meta_bankname + ' ' + this.Meta_statename +' IFSC & MICR Code on Homes247'
      });
  ...
  affordablePropDetails() {
    ...
    this.Service2.getcategoryblogs(id).subscribe(blogs => {
        this.Vastu_Shastra = blogs['blogcategory'];
    });
  }
  localitys: any;
  locality = [];
