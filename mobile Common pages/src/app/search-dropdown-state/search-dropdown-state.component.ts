...
@Component({
  ...
  templateUrl: 'search-dropdown-state.component.html',
  styleUrls: ['search-dropdown-state.component.css'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => SearchDropdownStateComponent),
      multi: true,
    },
  ],
})
export class SearchDropdownStateComponent implements ControlValueAccessor {
  list = [];
  temp_list = [];
  keyword = '';
  _img: any;
  ...
  ValuePAssing() {
    this.routeSub = this.activeroute.params.subscribe(params => {
      ...
      this.value = StateName.replace(/-/g, ' ');
    })
  }
  writeValue(value) {
    // if (value) {
