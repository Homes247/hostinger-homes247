...
changeText: boolean;
  ...
  GetAllCityName(){
    this.routeSub = this.activeroute.params.subscribe(params => {
      ...
      var BlocStatename = City[0]['state'].toLowerCase();
      if (BlocStatename === StateName) {
      } else {
        this.router.navigate(['/404'], { skipLocationChange: true });
      }
      });
  ...
  GetAllBranchName(){
    this.routeSub = this.activeroute.params.subscribe(params => {
      ...
      var BlockCityName = Branch[0]['city'].toLowerCase();
      if (BlockCityName === CityName) {
      } else {
        this.router.navigate(['/404'], { skipLocationChange: true });
      }
      });
  ...
  GetAllDetailsName(){
    this.routeSub = this.activeroute.params.subscribe(params => {
      ...
      var BlockBrancName = Branch[0]['branch'].toLowerCase();
      if (BlockBrancName === CityName) {
      } else {
        this.router.navigate(['/404'], { skipLocationChange: true });
      }
      ...
      //  
      this.urlchecking= this.current_Branch_name.replace(/-/g, ' ');
      if(this.urlchecking === Branch ){
      }else{
        this.router.navigate(['/404'], { skipLocationChange: true });
      }
      });
