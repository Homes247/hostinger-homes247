...
@Component({
  ...
  styleUrls: ['./bank-name.component.css']
})
export class BankNameComponent implements OnInit {
  ...
  GetAllStateName() {
    ...
    this.routeSub = this.activeroute.params.subscribe(params => {
      ...
      this.titleService.setTitle('Find IFSC | MICR Code For ' + this.Meta_bankname + ' | Homes247');
      this.meta.updateTag({
        name: 'description',
        content: `Find all the details for ${this.Meta_bankname} IFSC & MICR code in India at Homes247.in. Search branch-wise IFSC, MICR codes, addresses, and bank details for secure online transactions.`
      });


      this.Service.createLinkForCanonicalURL();

      if (this.router.url.indexOf('/all-ifsc-and-micr-code/') > -1) {
      } else {
        this.router.navigate(['/404'], { skipLocationChange: true });
      }
      if (this.router.url.indexOf('%20') > -1) {
        this.router.navigate(['/404'], { skipLocationChange: true });
      }
      ...
      this.Service2.IfscFetching(param).subscribe(lists => {
        ...
        var BlockBankname = State[0]['bank_name'].toLowerCase();
        if (BlockBankname === BankName) {
        } else {
          this.router.navigate(['/404'], { skipLocationChange: true });
        }
