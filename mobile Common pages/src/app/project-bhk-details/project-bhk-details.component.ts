...
import { emiloan, enquiry } from '../prop-details-new/class';
...
declare var $: any;
@Component({
  selector: 'app-project-bhk-details',
  templateUrl: './project-bhk-details.component.html',
  styleUrls: ['./project-bhk-details.component.css']
})
export class ProjectBhkDetailsComponent implements OnInit {
  ...
  constructor(
    ...
    public Service: DataService, public Service2: DataService2,
  ...
  ngOnInit() {
    this.activatedRoute.queryParamMap.subscribe(params => {
      this.propertyID = params['params']['property'];
      this.bhkID = params['params']['bhk'];
      this.dataloads(this.propertyID, this.bhkID);
  ...
  dataloads(propid, bhkId) {
    ...
    this.routeSub = this.activatedRoute.params.subscribe(params => {
      ...
      var lasturl = params['bhk-:size-:proptype-:locality-:cityname-:propid'];
      ...
      this.Service2.getpropertynew(propid).subscribe(prop => {
        ...
        // META DATA STARTS
        this.titleService.setTitle(propDetails[0].propertyName + ' ' + propDetails[0].locality_name + ', ' + propDetails[0].city_name + ' | Price, Reviews & Floorplans | Homes247.in');
        this.meta.updateTag({
          name: 'description',
          content: 'Explore ' + propDetails[0].propertyName + ' ' + propDetails[0].locality_name + ', ' + propDetails[0].city_name + ' Check out the Best ' + propDetails[0].propertyType + ', Price and Amenities'
        });
        this.meta.updateTag({
          property: 'og:image',
          content: this.Service.imagesURL + 'uploadPropertyImgs/' + propDetails[0].images[0].name
        });
        this.meta.updateTag({
          property: 'og:title',
          content: propDetails[0].propertyName + ' ' + propDetails[0].locality_name + ', ' + propDetails[0].city_name + ' | Price, Reviews & Floorplans'
        });
        this.meta.updateTag({
        ...
        this.video_URL = this.propertiesDetails[0]['video_URL'];
        if (this.video_URL === null) {
          this.VideoLinkHide = false;
        }
        ...
        this.faqsection1 = {
          'faqresults': [
            {
              'question': 'Where is ' + propname + ' located?',
              'answer': propname + ' is located at ' + localityname + ',' + cityname + '.'
            },
            {
              'question': 'What is the project status and possession time of ' + propname + ' ?',
              'answer': propname + ' Possession Time is ' + this.propertiesDetails[0].PossessionDate + ' , and the status is ' + this.propertiesDetails[0].Status + '.'
            },
            {
              'question': 'Is ' + propname + ' RERA Registered?',
              'answer': 'Yes, ' + propname + ' is RERA Registered.'
            },
          ]
        };
      });
      ...
      //BHK Details Api
      this.Service2.getBHKWiseDetails(bhkId).subscribe(data => {
        this.BHKDetails = data['BHK_Details'];

        this.BHKName = this.BHKDetails[0]['BHK'];
        this.AreaName = this.BHKDetails[0]['Area'];
        this.FloorImage = this.BHKDetails[0]['FloorImage'];
        this.PriceName = this.BHKDetails[0]['Price'];
        this.bathroomName = this.BHKDetails[0]['bathroom'];
      });
      //BHK Details Api
      ...
      this.Service2.get_amen_appro_banks(this.propID).subscribe(datadetails => {
        ...
        this.approvalsfaq = otherdatas[0]['Approvals_Details'].map(item => item.Name);
        this.bankfaq = otherdatas[0]['Bank_Details'].map(item => item.Name);
        this.bhkfilter = otherdatas[0].BHK_Details.map(item => item.BHK).filter((value, index, self) => self.indexOf(value) === index);
        ...
        this.faqsection3 = {
          'faqresults': [
            {
              'question': 'Which are the Regulatory Authorities that have approved ' + propname + ' ?',
              'answer': 'The Approving Authority for ' + propname + ' are ' + this.approvalsfaq + '.'
            },
            {
              'question': 'Which banks have approved loans for ' + propname + ' ?',
              'answer': 'These are the approved banks - ' + this.bankfaq + '.'
            },
            {
              'question': 'Which are the BHK configurations at ' + propname + ' ?',
              'answer': 'There are ' + this.bhkfilter + ' in ' + propname + '.'
            },
          ]
        };
      });
      ...
      this.Service.reviewfetching(this.propID).subscribe(response => {
        ...
        // Property Description and Main Fields Data Starts
        this.Service.getfields(this.propID).subscribe(fields => {
          ...
          this.faqsection2 = {
            'faqresults': [
              {
                'question': 'How many apartments does ' + propname + ' have?',
                'answer': 'There are ' + this.totalflats + ' apartments in ' + propname + '.'
              },

              {
                'question': 'What is the Price of ' + propname + ' ?',
                'answer': 'The price of ' + propname + ' ranges from ' + this.pricemin + ' to ' + this.pricemax + '.'
              },
            ]
          };
        });
      ...
      // Property Description and Main Fields Data Ends
  ...
  fontloads() {
    if (isPlatformBrowser(this.platformId)) {
      ...
      node4.href = 'https://d1zt14hr2k4poi.cloudfront.net/version2.0/proxima/stylesheet.css';
      node4.type = 'text/css';
      node4.async = true;
      var godefer = document.getElementsByTagName('link')[0];
      godefer.parentNode.insertBefore(node4, godefer);
  ...
  propenqSubmitForm() {
    ...
    //   {
    this.Service.EnqProperty(param, propNamepluscityname, this.cityid).subscribe(success => {
  ...
  SubmitFormByBhk() {
    var param = this.user;
    var propNamepluscityname = this.propName + '-' + this.Citynamelowcase;
    this.Service.EnqPropertyByBHKs(param, this.varient, propNamepluscityname, this.cityid).subscribe(success => {
  ...
  brochureSubmitForm() {
    ...
    // this.Service.EnqPropertyByBHKs(param, this.varient, this.propName).subscribe(success =>
    this.Service.EnqProperty(param, pageorgin, this.cityid).subscribe(success => {
      if (success['status'] === 'True') {
        this.otploader = false;
        this.cancel.nativeElement.click();
        $('body').removeClass('bodyoverlay');
  ...
  getintouch() {
    ...
    this.Service.specialformproppage(param, pageorgin, this.bhkselected, this.propertytypeselected,
      this.selectedPriceRange, this.userlocation, this.cityid).subscribe(success => {
