...
@Component({
  selector: 'app-inner-header-new',
  templateUrl: './inner-header-new.component.html',
  styleUrls: ['./inner-header-new.component.css']
})
export class InnerHeaderNewComponent implements OnInit {

  @ViewChild('scrollapiloader') scrollapiloader: ElementRef;
  userlogin = false;
  loginshow = true;
  HeaderpartValidate: boolean = true
  ...
  ngOnInit() {
    ...
    if (this.router.url.indexOf('/careers') > -1) {
      ...
      //     // }
      //   } else {
      //     console.log(self.HeaderpartValidate)
      //     if (self.HeaderpartValidate) {
      ...
      //   }
      // });
      } // close isPlatformBrowser else block
    } // close if/else careers
  ...
  bhk_status_type_locality_city() {
    ...
    this.routeSub = this.activeroute.params.subscribe(params => {
      ...
      var localityId = this.router.url.split('-').pop();
  ...
  Btlc() {
    ...
    var value = this.cityservice.cityfinder(this.router.url);
    var cityForMeta = value.cityname || 'Bangalore';
    this.Service.getlocalitymeta(cityForMeta, paramlocality).subscribe(metatag => {
      let metatags = metatag['Localityseo'];  //gowshik edit//
      this.localityName = metatags[0].LocalityName;
      this.localityName = this.localityName;
