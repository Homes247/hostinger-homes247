...
@Injectable()
export class ServerResponseService_bstlc {
  ...
  setStatus(code: number, noOfBedrooms?:number, City_Seo?:string, Locality_Seo?:string, localityid?:number): this {
    if (this.response) {
      this.response.redirect(code, "https://www.homes247.in/bstlc/"+ noOfBedrooms +"-bhk-ready-to-move-flats-apartments-in-"+ Locality_Seo +"-"+ City_Seo +"-"+ localityid);
    }
    return this;
  }
