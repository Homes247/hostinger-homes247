...
import { ServerResponseService_Locality_Overview } from '../server-response-locality-overview.service';
...
@Component({
  ...
  templateUrl: './locality-overview.component.html',
  styleUrls: ['./locality-overview.component.css'],
  providers: [ServerResponseService_Locality_Overview],

})
export class LocalityOverviewComponent implements OnInit {
  ...
  onTouchLoad() {
    import('../apex-chart2/apex-chart2.module').then(mod => mod.ApexChart2Module).then(ApexChart2Module => {
      this.Apex = ApexChart2Module.components['lazy'];
  ...
  dataloads() {
    ...
    const match = url.match(/overview-(\d+)-(\d+)/);

    if (match) {
      this.localityId = +match[1];
      this.localityIdTrends = this.localityId;
    }

    const value = this.cityservice.cityfinder(url);
    this.currentCity = value.cityname;
    this.cityId = value.cityid;
    this.city_Id = value.cityid;

    const paramlocality = { locid: this.localityId };
    ...
    this.Service.getlocalitymeta(this.currentCity, paramlocality).subscribe(metatag => {
      ...
      const localitySeo = this.currentLocalityName.toLowerCase().replace(/\s+/g, '-');
      const citySeo = this.currentPropCity.toLowerCase().replace(/\s+/g, '-');
      const expectedUrl = `/locality-overview/${localitySeo}-in-${citySeo}-overview-${this.localityId}-${this.city_Id}`;

      const title = this.getSeoTitle(
        `${this.currentLocalityName} Real Estate Guide | Homes247.in`,
        `${this.currentLocalityName} Real Estate Guide`
      );

      const description = `Explore the ${this.currentLocalityName} property market. Get real estate trends and top residential projects at Homes247.in.`;
      ...
      if (url.includes('--') || cleanUrl !== expectedUrl) {
        this.ServerResponseService_locality_overview.set301Status(localitySeo, citySeo, this.localityId, this.city_Id);
      }
    ...
    //     this.localityIdTrends = locality_id;
    //   } else {
    ...
    //   var Locality_seo = this.currentLocalityName.toLowerCase().replace(/\s+/g, '-');
    //   var City_seo = this.currentPropCity.toLowerCase().replace(/\s+/g, '-');
    ...
    //   this.meta.updateTag({ name: 'description', content: "Know all about" + this.currentLocalityName + ", " + this.currentPropCity + ": Check " + this.currentLocalityName + " map, photos, reviews, residential places, schools, & hospitals." });
    //   this.Service.createLinkForCanonicalURL();


    //  var urlstructure = "/locality-overview/"+Locality_seo+"-in-"+City_seo+"-overview-"+this.localityId+"-"+this.city_Id


    //   if (this.router.url.indexOf('--') > -1) {
    //     this.ServerResponseService_locality_overview.set301Status(Locality_seo,City_seo,this.localityId,this.city_Id);
    //   } else if (this.router.url.split('?')[0] === urlstructure) {
    //   }  else{
    //     this.ServerResponseService_locality_overview.set301Status(Locality_seo,City_seo,this.localityId,this.city_Id);
    //   }
    //     // this.titleService.setTitle(this.currentLocalityNam e +',' + this.currentPropCity + '| Locality Reviews & Insights!');
    //     // this.meta.updateTag({
    //     //   name: 'description',
