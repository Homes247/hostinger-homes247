import { Component, Input, forwardRef, HostListener, ElementRef, Output, EventEmitter, ViewChild, Inject, } from '@angular/core';
...
declare var $: any;
@Component({
  selector: 'search-dropdown-branch',
  templateUrl: 'search-dropdown-branch.component.html',
  styleUrls: ['search-dropdown-branch.component.css'],
  providers: [
    {
      provide: NG_VALUE_ACCESSOR,
      useExisting: forwardRef(() => SearchDropdownBranchComponent),
      multi: true,
    },
  ],
})
export class SearchDropdownBranchComponent implements ControlValueAccessor {
  list = [];
  temp_list = [];
  keyword = '';
  _img: any;
  ...
  constructor(private ele: ElementRef, @Inject(LOCAL_STORAGE) private Local_Storage: any, private activeroute: ActivatedRoute,) { }
  ngOnChanges() {
    ...
    this.value = this.Local_Storage.getItem('Branch_Name');
    this.ValuePAssing()
  }
