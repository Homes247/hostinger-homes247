...
declare var $: any;
@Component({
  selector: 'app-bank-ifsc-branch',
  templateUrl: './bank-ifsc-branch.component.html',
  styleUrls: ['./bank-ifsc-branch.component.css']
})
export class IfscBranchComponent implements OnInit {
...
changeText: boolean;
  ...
  GetAllBranchName(){
    this.routeSub = this.activeroute.params.subscribe(params => {
      ...
      this.titleService.setTitle('Find IFSC & MICR Code For ' + this.Meta_bankname +' in ' + this.Meta_cityname);
      this.meta.updateTag({
        name: 'description',
        content: 'Find Details of ' + this.Meta_bankname + ' ' + this.Meta_cityname +' IFSC & MICR Code on Homes247'
      });
      ...
      this.Local_Storage.setItem('Branch_Name', 'Select');
        var param = {
        Bank : BankName,
        State: StateName,
        City:  CityName,
  ...
  affordablePropDetails() {
    ...
    this.Indian_Festivals = blogs['blogcategory'];
    });
  }
  localitys: any;
  locality = [];
