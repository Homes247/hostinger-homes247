import { Component, OnInit, ViewChild, PLATFORM_ID, Inject, ChangeDetectorRef } from '@angular/core';
import { FormControl, ReactiveFormsModule } from '@angular/forms'; // Added ReactiveFormsModule
import { ActivatedRoute, Router, RouterModule } from '@angular/router'; // Added RouterModule
import { CommonModule } from '@angular/common'; // Added CommonModule for pipes/directives
...
export class EjsAutocomplete {
  ...
  ngOnInit(): void {
    if (isPlatformBrowser(this.platformId)) {
      ...
      //     // ✅ Only start filtering when length >= 2
      //     if (filterValue.length < 2) {
      //       return [];
      //     }

      //     // ✅ Only show words starting with typed value
  ...
  //   this.empty = this.options.filter(option => option.name.toLowerCase().startsWith(filterValue).length)
  //   // alert(this.empty)
  ...
  //       this.autoCompleteData = myLocalList as AutoCompleteItem[];
  //       // 
  //     } else {
  ...
  getAutocomp() {
    ...
    if (this.wheretotakecity == false) {
      ...
      var finalcitydata = this.storage.getItem('CityName');
    }
