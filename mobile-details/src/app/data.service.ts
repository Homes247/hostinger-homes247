...
export class DataService {
  ...
  private REST_API_SERVER2_Hari = 'http://192.168.0.119/right2shout_LIVE/Backend/';
  ...
  getTechnologyList(id) {
    ...
    const req = this.httpClient.get(this.REST_API_SERVER22 + 'blogcategoriesnew', options).pipe(retry(0), catchError(this.handleError));
  ...
  commercialSaleProperties(city, param) {
    const req = this.httpClient.get(this.Commercial + 'listing/' + city + '?', { params: toHttpParams(param) }).pipe(retry(0), catchError(this.handleError));
  ...
  PGRent(city, param) {
    const req = this.httpClient.get(this.PG + 'listing/' + city + '?', { params: toHttpParams(param) }).pipe(retry(0), catchError(this.handleError));
  ...
  getAuto(city_id: string): Observable<any> {
    ...
    if (this.lastCityId === city_id) { return this.autoCache[key] || of(null); }
    if (this.autoCache[key]) { return this.autoCache[key]; }
    if (this.apiCallStatus[key]) { return this.apiCallStatus[key].asObservable(); }

    this.apiCallStatus[key] = new Subject<any>();
    const options = { params: new HttpParams().set('city_id', city_id) };

    // Wrap the core HTTP call for SSR State
    const apiRequest = this.srv(key, this.httpClient.get(this.REST_API_SERVER + 'updatedautocomplete?', options).pipe(
      retry(1), shareReplay(1), catchError(() => of(null))
    ), null);
  ...
  getlocalityAuto(city_id) {
    const options = { params: new HttpParams({ fromString: 'city_id=' + city_id }) };
    const req = this.httpClient.get(this.REST_API_SERVER + 'locality_autocomplete?', options).pipe(retry(0), catchError(this.handleError));
    return this.srv(`loc_auto_${city_id}`, req, []);
  }

  getlocalityreverseAuto(city_id) {
    const options = { params: new HttpParams({ fromString: 'city_id=' + city_id }) };
    const req = this.httpClient.get(this.REST_API_SERVER + 'localityreverse_autocomplete?', options).pipe(retry(0), catchError(this.handleError));
    return this.srv(`loc_rev_${city_id}`, req, []);
  }

  private builderCache: { [city_id: string]: Observable<any> } = {};
  private lastBuilderCityId: string | null = null;

  getbuilderAuto(city_id: string): Observable<any> {
    const key = `builder_autocomplete_${city_id}`;
    if (this.lastBuilderCityId === city_id) { return this.builderCache[key] || of(null); }
    if (this.builderCache[key]) { return this.builderCache[key]; }

    const options = { params: new HttpParams().set('city_id', city_id) };
    const apiRequest = this.srv(key, this.httpClient.get(this.REST_API_SERVER + 'builder_autocomplete?', options).pipe(
      retry(1), shareReplay(1), catchError(() => of(null))
    ), null);

    this.builderCache[key] = apiRequest;
    this.lastBuilderCityId = city_id;
    return apiRequest;
  }
  ...
  private lastLocalityMetaKey: string | null = null;
  ...
  getlocalitymeta(city: string, param: { locid: any }): Observable<any> {
    ...
    if (this.lastLocalityMetaKey === cacheKey) { return this.localityMetaCache[cacheKey] || of(null); }
    if (this.localityMetaCache[cacheKey]) { return this.localityMetaCache[cacheKey]; }

    const options = new HttpParams().set('locid', locidStr);
    const apiRequest = this.srv(cacheKey, this.httpClient.get(this.REST_API_SERVER + 'get_locseotag/' + city + '?', { params: options }).pipe(
      retry(0), shareReplay(1), catchError(() => of(null))
    ), null);

    this.localityMetaCache[cacheKey] = apiRequest;
    this.lastLocalityMetaKey = cacheKey;
    return apiRequest;
  }
  ...
  getcityregionmeta(regionid) {
    ...
    const req = this.httpClient.get(this.REST_API_SERVER + 'cityregionseo?', options).pipe(retry(0), catchError(this.handleError));
  ...
  private localityCache: { [city_id: string]: Observable<any> } = {};
  private lastLocalityCityId: string | null = null;

  getTopLocalities(param: { cityId: string }): Observable<any> {
    const city_id = param.cityId;
    const key = `toplocalities${city_id}`;
    if (this.lastLocalityCityId === city_id) { return this.localityCache[key] || of(null); }
    if (this.localityCache[key]) { return this.localityCache[key]; }

    const apiRequest = this.srv(key, this.httpClient.get(this.REST_API_SERVER + 'top_localities?', { params: toHttpParams(param) }).pipe(
      retry(0), shareReplay(1), catchError(() => of(null))
    ), null);

    this.localityCache[key] = apiRequest;
    this.lastLocalityCityId = city_id;
    return apiRequest;
  }
  ...
  Loginwithnum(param) {
    ...
    const req = this.httpClient.post(this.REST_API_SERVER2 + 'login_withnumber_hari', options).pipe(retry(0), catchError(this.handleError));
  ...
  getIndividualList(citi, param) {
    const req = this.httpClient.get(this.REST_API_SERVER_Individuallist3 + 'listings/' + citi + '?', { params: toHttpParams(param) }).pipe(retry(0), catchError(this.handleError));
  ...
  getrentalList(city, param) {
    const req = this.httpClient.get(this.REST_API_SERVER_rentals + 'listings/' + city + '?', { params: toHttpParams(param) }).pipe(retry(0), catchError(this.handleError));
  ...
  getrentfilterslistFilter(cityId: string): Observable<any> {
    const key = `rent_filters_${cityId}`;
    if (this.lastRentFiltersCityId === cityId) { return this.rentFiltersCache[key] || of(null); }
    if (this.rentFiltersCache[key]) { return this.rentFiltersCache[key]; }

    const apiRequest = this.srv(key, this.httpClient.get(this.REST_API_SERVER_rentals + 'datafilters', { params: new HttpParams().set('CityId', cityId) }).pipe(
      retry(0), shareReplay(1), catchError(() => of(null))
    ), null);

    this.rentFiltersCache[key] = apiRequest;
    this.lastRentFiltersCityId = cityId;
    return apiRequest;
  }

  private rentFiltersCache: { [cityId: string]: Observable<any> } = {};
  private lastRentFiltersCityId: string | null = null;
  ...
  getFiltersDatalistSaleTest(cityId: string): Observable<any> {
    const key = `filters_datalist_${cityId}`;
    if (this.lastFiltersCityId === cityId) { return this.filtersCache[key] || of(null); }
    if (this.filtersCache[key]) { return this.filtersCache[key]; }

    const apiRequest = this.srv(key, this.httpClient.get(this.REST_API_SERVER + 'datafilters', { params: new HttpParams().set('CityId', cityId) }).pipe(
      retry(0), shareReplay(1), catchError(() => of(null))
    ), null);

    this.filtersCache[key] = apiRequest;
    this.lastFiltersCityId = cityId;
    return apiRequest;
  }

  private filtersCache: { [cityId: string]: Observable<any> } = {};
  private lastFiltersCityId: string | null = null;
