import { AfterViewInit, Component, DOCUMENT, HostListener, Inject, OnInit, PLATFORM_ID } from '@angular/core';
...
@Component({
  selector: 'app-inner-headder-with-sidenav',
  imports: [CommonModule, RouterModule, cleanUrlPipe, MyBHKPipe],
  templateUrl: './inner-headder-with-sidenav.html',
  styleUrl: './inner-headder-with-sidenav.css',
})


export class InnerHeadderWithSidenav implements OnInit, AfterViewInit {
  ...
  subSideNavBarContents() {
    ...
    if (value.cityid === undefined) {
      ...
      this.storage.setItem('CityName', this.currentCity);
    }
...
onLogin
  ...
  ngAfterViewInit(): void {
    ...
    this.determineContext(this.router.url);

    // 2. Listen for navigation changes (when user clicks a link)
    this.router.events.pipe(
      filter(event => event instanceof NavigationEnd)
    ).subscribe((event: any) => {
      this.determineContext(event.urlAfterRedirects);
    });


    if (this.pageContext === 'locality') {
    this.fetchLocality()
    }
    if (this.pageContext === 'builder') {
    this.fetchBuilder()
    }



    this.Service.getZoneList(this.cityid).subscribe((zoneList) => {
      if (zoneList['status'] == 'True') {
        this.zonesLists = zoneList['result']
        this.zoneListPresent = true
      }
    })

  }
  zonesLists: any[] = []
  zoneListPresent: any = false

  pageContext: 'city' | 'locality' | 'builder' = 'city';

  determineContext(url: string) {
    const currentUrl = url.toLowerCase();

    const builderPrefixes = [
      '/builder/', '/brtc/', '/btbc/', '/bltc/', '/blpc/',
      '/bapc/', '/batc/', '/bbc/', '/spbc/', '/stbc/',
      '/bplc/', '/baplc/', '/batlc/', '/blplc/', '/bltlc/'
    ];

    const localityPrefixes = [
      '/btlc/', '/btllc/', '/ltlc/', '/lplc/', '/atlc/',
      '/aplc/', '/btalc/', '/stlc/', '/bstlc/', 'property-sale-in-'
    ];

    const isBuilder = builderPrefixes.some(prefix => currentUrl.includes(prefix));

    const isLocality = localityPrefixes.some(prefix => currentUrl.includes(prefix));

    if (isBuilder) {
      this.pageContext = 'builder';
    } else if (isLocality) {
      this.pageContext = 'locality';
    } else {
      this.pageContext = 'city';
    }
  }
  ...
  fetchLocality() {
    ...
    this.localityId = localityId_1
    var paramlocality = {
      locid: localityId_1,
    };
    this.Service.getlocalitymeta(this.cityname, paramlocality).subscribe({
      next: (metatag: any) => {
        const metatags = metatag && metatag['Localityseo'];
        if (Array.isArray(metatags) && metatags.length > 0 && metatags[0].LocalityName) {
          this.localityName = metatags[0].LocalityName;
        } else {
          this.localityName = '';
        }
  ...
  closeAndroidApp() {
    if (isPlatformBrowser(this.platformId)) {
      ...
      $('.topMarginBreadCrumbs').css('height', '68px');
