...
export class InnerHeader implements AfterViewInit {
  ...
  ngAfterViewInit() {
    ...
    // inner-header.ts ngAfterViewInit
  ...
  getAutocomp() {
    ...
    // });
    this.Service.getlocationlist().pipe(take(1)).subscribe((city) => {
      if (city['status'] === 'True') this.citiess = city['locations'];
    });

    this.dataService2.getmajorcities().pipe(take(1)).subscribe((majorcity: any) => {
      if (majorcity['status'] === 'True') this.majorcities = majorcity['locations'];
    });
    ...
    this.Service.getlocalityAuto(this.cityid)
      .subscribe((res: any) => {
        ...
        // console.log('Locality Auto Complete Data:', this.autoCompleteData_loc);
      });
    ...
    this.Service.getbuilderAuto(this.cityid)
      .subscribe((res: any) => {
        ...
        // console.log('Builder Auto Complete Data:', this.autoCompleteData_build);
      });
  }
