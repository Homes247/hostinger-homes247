...
// import { City, flitercity, minmax } from '../locality/locality';
...
@Component({
  selector: 'app-individual-locality-list',
  imports: [
    CommonModule,
    FormsModule,
    ReactiveFormsModule,
  ...
  templateUrl: './individual-locality-list.html',
  styleUrl: './individual-locality-list.css',
})
export class IndividualLocalityList implements OnInit, AfterViewInit, OnDestroy {
  ...
  listarraylength: any;
  urlparam: any;
  localityIdforUrl: any;
  localityNameForUrl: any;
  cityNameForUrl: any;
  localityname: any;
  propertiescount: any;
  propbhk: any;
  ...
  ngOnInit() {
    ...
    IndividualLocalityList.citycount = 0;
  ...
  getindividuallist() {
    IndividualLocalityList.citycount = 4;
    this.routeSub = this.activeroute.params.subscribe(params => {
      // var citiname = params['cityname'];
      var url = params['projects-for-sale-in-:localityname-:cityname-:localityid'];
      this.urlparam = url;
      var localityid = url.split('-').pop().match(/[0-9]+/);
      this.localityIdforUrl = localityid;
      var limit = '0';
      var limitrows = '4';
      var limitparam = 0;
      ...
      var param1 = { locality: localityid };
      // console.log(param);
      this.Service.getindividualprojectscount(this.cityname, param).subscribe(countprojects => {
        // this.showLoader = true;
        let projectcount = countprojects['Counts'];
        this.propertiescount = projectcount[0].PropertyCounts;
      });

      this.Service.getprojectscount(this.city, param1).subscribe(countprojects => {
        let projectcount = countprojects['Counts'];
      ...
      this.Service.commercialSalePropertiesCount(this.city, param2).subscribe(countprojects => {
        let projectcount = countprojects['Counts'];
      ...
      this.Service.getIndividualList(this.cityname, param).subscribe(lists => {
  ...
  loadMore() {
    this.routeSub = this.activeroute.params.subscribe(params => {
      ...
      // const limit = IndividualLocalityList.citycount += 4;
      const limit = IndividualLocalityList.citycount;
      IndividualLocalityList.citycount += 4;
      ...
      var param = {
        ...
        minprice: min,
        maxprice: max,
        locality: this.localityIdforUrl,
        balcony: balcony,
        furnish: Furnish,
        ownership: posted,
        doorface: doorfacing,
      ...
      if (livecount < totalcount) {
        return this.Service.getIndividualList(this.cityname, param).subscribe(propertylists => {
  ...
  clickprojects() {
    ...
    this.router.navigate([this.cityNameForUrl + '/' + staticlocurl + '-' + this.localityNameForUrl + '-' + this.localityIdforUrl]);
  }
  ...
  SubmitForm() {
    ...
    var propertyname = this.propbhk + '-' + this.proptype + '-' + this.propname;
  ...
  CompareNow() {
    ...
    this.storage.setItem('page_type', 'AllLocality')
